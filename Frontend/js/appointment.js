        ...
        const APPOINTMENT_API_BASE = 'http://localhost:5001/api/appointments';
        ...
        function setupNearbyProfessionals() {
            const findBtn = document.getElementById('findNearbyBtn');
            if (!findBtn) return;
            findBtn.addEventListener('click', findNearbyProfessionals);
        }

        function lookupCityFromCoordinates(lat, lng) {
            // Mock lookup (Chennai/Tamil Nadu) as required.
            if (
                Number.isFinite(lat) && Number.isFinite(lng) &&
                lat >= 12.80 && lat <= 13.25 &&
                lng >= 79.95 && lng <= 80.35
            ) {
                return { city: 'Chennai', state: 'Tamil Nadu', source: 'frontend_mock' };
            }
            return { city: 'Chennai', state: 'Tamil Nadu', source: 'frontend_default' };
        }

        async function findNearbyProfessionals() {
            const grid = document.getElementById('nearbyProfessionalsGrid');
            const badge = document.getElementById('nearbyLocationBadge');
            const findBtn = document.getElementById('findNearbyBtn');
            if (!grid || !badge || !findBtn) return;

            findBtn.disabled = true;
            findBtn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Detecting Location...';
            grid.innerHTML = `
                <div class="loading-appointments">
                    <div class="loading-spinner"></div>
                    <h3>Finding nearby professionals...</h3>
            ...
            try {
                ...
                const cityInfo = lookupCityFromCoordinates(latitude, longitude);

                const response = await fetch(
                    `${APPOINTMENT_API_BASE}/nearby-professionals?lat=${encodeURIComponent(latitude)}&lng=${encodeURIComponent(longitude)}&city=${encodeURIComponent(cityInfo.city)}&state=${encodeURIComponent(cityInfo.state)}`,
                    {
                        method: 'GET',
                ...
                const data = await response.json();
                if (!data.success) {
                    throw new Error(data.message || 'Unable to fetch nearby professionals');
                }

                const location = data.location || cityInfo;
                badge.textContent = `Location: ${location.city}, ${location.state}`;
                renderNearbyProfessionals(data.professionals || []);
            } catch (error) {
                console.error('Nearby professionals lookup failed:', error);
                badge.textContent = 'Location unavailable - showing Chennai, Tamil Nadu';
                await loadNearbyByFallbackCity();
            } finally {
                findBtn.disabled = false;
                findBtn.innerHTML = '<i class="fas fa-location-crosshairs"></i> Find Near Me';
            }
        }
        ...
        async function loadNearbyByFallbackCity() {
            const grid = document.getElementById('nearbyProfessionalsGrid');
            if (!grid) return;
            try {
                const response = await fetch(
                    `${APPOINTMENT_API_BASE}/nearby-professionals?city=${encodeURIComponent('Chennai')}&state=${encodeURIComponent('Tamil Nadu')}`
                );
                const data = await response.json();
                renderNearbyProfessionals((data && data.professionals) || []);
            } catch (fallbackError) {
                console.error('Fallback nearby lookup failed:', fallbackError);
                renderNearbyProfessionals([]);
            }
        }

        function renderNearbyProfessionals(professionals) {
            const grid = document.getElementById('nearbyProfessionalsGrid');
            if (!grid) return;
            ...
            if (!Array.isArray(professionals) || professionals.length === 0) {
                grid.innerHTML = `
                    <div class="no-appointments">
                        ...
                        <h3>No nearby professionals found</h3>
                        <p>Try again in a few minutes or contact Soul Space support.</p>
                    </div>
                `;
                return;
            }
            ...
            grid.innerHTML = professionals.map((pro) => `
                <div class="appointment-card confirmed" style="border-left: 4px solid #22c55e;">
                    <div class="appointment-header">
                        <div class="appointment-specialist">
                            <div class="appointment-avatar">${pro.name.split(' ').map(n => n[0]).slice(0,2).join('')}</div>
                            <div class="appointment-specialist-info">
                                <h4>${pro.name}</h4>
                                <div class="appointment-specialist-role">${pro.role}</div>
                            </div>
                        </div>
                        <div class="appointment-status confirmed">verified</div>
                    </div>
                    <div class="appointment-details">
                        <div class="appointment-detail">
                            <div class="appointment-detail-icon"><i class="fas fa-stethoscope"></i></div>
                            <div class="appointment-detail-info">
                                <div class="appointment-detail-label">Specialty</div>
                                <div class="appointment-detail-value">${pro.specialty}</div>
                            </div>
                        </div>
                        <div class="appointment-detail">
                            <div class="appointment-detail-icon"><i class="fas fa-location-dot"></i></div>
                            <div class="appointment-detail-info">
                                <div class="appointment-detail-label">Location</div>
                                <div class="appointment-detail-value">${pro.city}, ${pro.state}</div>
                            </div>
                        </div>
                        <div class="appointment-detail">
                            <div class="appointment-detail-icon"><i class="fas fa-award"></i></div>
                            <div class="appointment-detail-info">
                                <div class="appointment-detail-label">Experience</div>
                                <div class="appointment-detail-value">${pro.experience}+ years</div>
                            </div>
                        </div>
                    </div>
                    <div class="appointment-actions">
                        <a class="appointment-btn appointment-btn-primary" href="${pro.whatsappLink}" target="_blank" rel="noopener noreferrer">
